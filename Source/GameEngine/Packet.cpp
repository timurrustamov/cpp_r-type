...
Packet::fromStream(std::vector<unsigned char> &data)
{
	...
	if (*r_magic == 0)
	{
		...
		r_magic = reinterpret_cast<unsigned int *>(&data[0]);
	}
	...
	data.erase(data.begin(), data.begin() + sizeof(unsigned int) * 3 + *r_size);
...
Packet::build()
{
	...
	//allocate
	std::vector<unsigned char> *build = new std::vector<unsigned char>(sizeof(unsigned int) * 3);
	for (unsigned int i = 0; i < sizeof(unsigned int); i++) {
